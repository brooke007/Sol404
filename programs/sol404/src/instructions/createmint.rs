...
const ADMIN_PUBKEY: Pubkey = pubkey!("7yPgZvD4bQtHaZ8DB8Bijta6YTJSjMnvjbrc447HttLX");
...
pub fn createmint(
    ...
    ctx.accounts.init_data.token_price = 10;
    ...
    ctx.accounts.init_data.token_price = 1;
    ...
    ctx.accounts.init_data.token_price = 0;
...
pub struct CreateMint<'info> {
    ...
    /// will be performed on it by Anchor. This is acceptable in this context because we manually ensure that
    /// the address is correct by deriving it from the USDC token mint's PDA (Program Derived Address). The
    /// responsibility to ensure that the account is valid and fulfills the required conditions for the
    /// operations performed on it is handled manually.
